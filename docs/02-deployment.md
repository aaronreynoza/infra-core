...
```bash
# Proxmox credentials
export PROXMOX_VE_ENDPOINT="https://REDACTED_MGMT_IP0:8006/api2/json"
...
curl -k https://REDACTED_MGMT_IP0:8006/api2/json/version

# Verify API token
curl -k -H "Authorization: PVEAPIToken=terraform@pve!terraform=xxx" \
  https://REDACTED_MGMT_IP0:8006/api2/json/version
```
...
```bash
# Check node status
talosctl -n REDACTED_K8S_API get members

# View logs
talosctl -n REDACTED_K8S_API logs controller-runtime

# Reset and retry
talosctl -n REDACTED_K8S_API reset --graceful=false
```
...
talosctl health --nodes REDACTED_K8S_API,10.10.10.11,10.10.10.20,10.10.10.21

# Check etcd status
talosctl -n REDACTED_K8S_API etcd status
